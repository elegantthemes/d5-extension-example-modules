...
import { StaticModuleEdit as ModuleComponent } from '../edit';
...
let StaticModuleEdit: typeof ModuleComponent;
...
beforeAll(() => {
  ...
  StaticModuleEdit = select('divi/module-library').getModule('example/static-module').renderers.edit;
});
...
describe.each(testCases as unknown as TestCase[])('<StaticModuleEdit />', ({ title, args }: TestCase) => {
  it(`Should match the snapshot when "${title}"`, () => {
    const component = createElement(StaticModuleEdit, args);
