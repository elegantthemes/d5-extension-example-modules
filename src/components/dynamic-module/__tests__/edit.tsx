...
import { DynamicModuleEdit as ModuleComponent } from '../edit';
...
let DynamicModuleEdit: typeof ModuleComponent;
...
beforeAll(() => {
  ...
  DynamicModuleEdit = select('divi/module-library').getModule('example/dynamic-module').renderers.edit;
});
...
describe.each(testCases as unknown as TestCase[])('<DynamicModuleEdit />', ({ title, args }: TestCase) => {
  it(`Should match the snapshot when "${title}"`, () => {
    const component = createElement(DynamicModuleEdit, args);
